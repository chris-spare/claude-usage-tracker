#!/usr/bin/env bash
# Build a distributable, notarized "Claude Usage.app".
#
...
#
# Output: build/Claude Usage.app  and  build/ClaudeUsage.zip (the artifact to
...
BIN="$(swift build -c release --arch arm64 --arch x86_64 --show-bin-path)/ClaudeUsageTray"
...
codesign --force --sign "$IDENTITY" --identifier "$BUNDLE_ID" \
    --entitlements Resources/ClaudeUsageTray.entitlements \
...
echo "arch: $(lipo -archs "$APP/Contents/MacOS/ClaudeUsageTray")"
